...
| 10. `flexkv/transfer/worker.py` 里曾在 `_transfer_impl` 后 `torch.cuda.synchronize()` | 把 D2H/H2D op 串行成 device-wide drain，R2 mean / p95 / p99 各 ~10-12% degrade | 已删除。下游 worker 队列已自带 stream-aware 排序；多余的 device sync 没有保护任何 invariant。NIXL worker 的 sync（`worker.py:1621`，由 PR #142 引入）属于另一条路径，与本项无关 |
